...
<script>
  ...
  function deleteComment(notopic,nocomment){
    ...
    $.post('functionWebboard.php',{deleteComment:"true",user:user,addLog:"true",notopic:notopic,nocomment:nocomment},
    function(data){
        location.reload();
    });
    }
  ...
  function addComment(notopic){
    ...
    //   console.log(comment);
        ...
        //  $db->create_log($_POST['user'],date('Y/m/d H:i:s'),action);
        $.post('functionWebboard.php',{addCreateLog:"true",addComment:"true",user:user,topic:topic,notopic:notopic,comment:comment},
         function(data){
        location.reload();
        });
